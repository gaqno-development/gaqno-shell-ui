...
// Requires: shell and AI MFE (gaqno-ai-ui) running and federated so /ai/images and /ai/audio load the MFE.
...
test.describe("AI Audio and Images child pages", () => {
  ...
  test("/ai/images/text shows Texto para Imagem as its own page", async ({
    ...
    await expect(
      page.getByRole("button", { name: "Gerar imagem" })
    ).toBeVisible();
  });
...
test.describe("AI Video child pages", () => {
  ...
  test("/ai/video/text shows Texto para Vídeo as its own page", async ({
    ...
    await expect(
      page.getByRole("button", { name: "Gerar vídeo" })
    ).toBeVisible();
  });
});
